...
// 页面自身的发布地址与分享图。两者都可选：不传就不输出 canonical/og:url/og:image，
// 而不是拿一个猜的地址填进去。
const siteFlag = process.argv.indexOf("--site-url");
const siteUrl = siteFlag >= 0 ? process.argv[siteFlag + 1].replace(/\/+$/, "") : undefined;
const ogImageFlag = process.argv.indexOf("--og-image");
const ogImage = ogImageFlag >= 0 ? process.argv[ogImageFlag + 1] : undefined;

// 被画的那个仓库：优先从 --repo-url 取 owner/repo，退回输出文件名。
const repoLabel = (() => {
...
const pageUrl = siteUrl ? `${siteUrl}/${outPath.split("/").pop()}` : undefined;
// 标题要能区分同一个仓库的不同页面，否则 arch / impact / change 三页同名。
const pageKind = impactTarget ? "blast-radius map" : diffBase ? "change map" : "architecture map";
const pageTitle = `${repoLabel} — ${pageKind}`;
const pageDesc = `Interactive architecture map of ${repoLabel} generated by codeblast: a module-level`
  + ` dependency graph where hovering a node lights up its fan-in and clicking it shows its files,`
  + ` symbols and import edges.`
  + (impactTarget ? ` Includes the blast-radius overlay for ${impactTarget}.` : "")
  + (overlayPath ? " Includes the change overlay." : "");
